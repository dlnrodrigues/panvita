...
def getMeta(a):
	os.system("which mlst > temp.temp")
	st = open("temp.temp", "rt")
	mlst = st.readlines()
	st.close()
	try:
		mlst = mlst[0].replace("\n", "")
	...
	out = open(a, "w")
	if mlst == "":
		out.write("Strain;Host;Host Disease;Geografic Localization;Isolation Source;Genomic Size;GC%;Release Date;BioSample\n")
	else:
		out.write("Strain;Host;Host Disease;Geografic Localization;Isolation Source;Genomic Size;GC%;Release Date;ST;BioSample\n")
	for i in dic2.keys():
...
def getNCBI_FNA():
	os.system("which prokka > temp.temp")
	pt = open("temp.temp", "rt")
	try:
		prokka = pt.readlines()[0].replace("\n", "")
	except:
		prokka = ""
	pt.close()
	pkgbf = []
...
if "-m" in sys.argv:
	prit("mlst\thttps://doi.org/10.1186/1471-2105-11-595")
	prit("mlst\thttps://github.com/tseemann/mlst"
if "-a" in sys.argv:
	prit("prokka\thttps://doi.org/10.1093/bioinformatics/btu153")
print('')
