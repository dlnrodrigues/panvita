			...
			for j in range(i, len(cds)):
				if "/locus_tag=" in cds[j]:
					...
					while ("   CDS   " not in cds[k]):
						if ("   gene" not in cds[k]) and ("locus_tag" not in cds[k]) and ("gene=" not in cds[k]) and ("assembly_gap" not in cds[k]):
							seq = cds[k].replace(" ", "")
							seq = seq.replace("\"", "")
							sequence.append(seq)
						if ("\"" in cds[k]) or ("ORIGIN" in cds[k]) or ("assembly_gap" in cds[k]) or ("gene" in cds[k]):
							break
						k = k + 1
	...
	for i in strains:
		...
		for j in final.keys():
			if "-card" == p:
				...
				color = "yellow\n"
			out.write(final[j][0]+'\t'+final[j][1][0]+'\t'+final[j][1][1][:-1]+'\t'+color)
	out.close()
#################################################Gerar positions#################################################
...
#################################################Gerar figura###################################################
	...
	x = math.ceil(len(headers)/1)
	y = math.ceil(len(lines)/1)
	print("\nPlotting final clustermap...")
	p2 = sns.clustermap(df, figsize=(50,60), cmap=color)
	p2.savefig(out, format='png', dpi=300, bbox_inches="tight")
#################################################Gerar figura###################################################
